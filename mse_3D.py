...
def run_c_program(csv_path, scales, m, r, delta, fuzzy, composite, distance_type, refined):
    ...
    command = ['./mse_3D', csv_path, str(num_files), str(scales), str(csv_rows), str(csv_cols), str(m), str(r_std), str(delta), str(fuzzy), str(composite), str(distance_type), str(refined)]
...
def mse_3D(folder_path, scales, m, r, delta, fuzzy=False, composite=False, distance_type=0, refined=False):
    if fuzzy == False:
        fuzzy = 0
    elif fuzzy == True:
        fuzzy = 1
    ...
    for filename in os.listdir(folder_path):
        ...
        mse_values.append([filename, run_c_program(file_path, scales, m, r, delta, fuzzy, composite, distance_type, refined)])
...
# v = mse_3D(folder_path='Datos/10x10x100/datos_csv', scales=20, m=1, r=0.5, delta=0.8, fuzzy=False, composite=False, distance_type=0, refined=False)
# plot_arrays(v)
