...
def run_c_program(csv_path, scales, m, r, delta, fuzzy):
    ...
    command = ['./mse_3D', csv_path, str(num_files), str(scales), str(csv_rows), str(csv_cols), str(m), str(r_std), str(delta), str(fuzzy)]
...
def mse_3D(folder_path, scales, m, r, delta, fuzzy=False):
    if fuzzy == False:
        fuzzy = 0
    elif fuzzy == True:
        fuzzy = 1
    ...
    for filename in os.listdir(folder_path):
        ...
        mse_values.append([filename, run_c_program(file_path, scales, m, r, delta, fuzzy)])
...
# v = mse_3D('Datos/10x10x100/datos_csv', 30, 1, 0.5, 0.8, fuzzy=False)
# plot_arrays(v)
