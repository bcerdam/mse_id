...
float calculate_U_ij_m(double ***list_of_matrices, int i, int j, int k, int m, double r, int H, int W, int T, double delta, int fuzzy, int distance_type, int window_size) {
    double count = 0;
    int N_m = (H - m) * (W - m) * (T - m);
    for (int c = 0; c < T - m; c++){
        for (int a = 0; a < H - m; a++) {
...
float calculate_U_m(double ***list_of_matrices, int m, double r, int H, int W, int T, double delta, int fuzzy, int distance_type, int window_size) {
    double sum = 0.0;
...
double* method_mse(double*** list_of_matrices, int scales, int m, double r, int fuzzy, int method, double delta, int distance_type, int num_files, int rows, int cols){
    ...
    for (int i = 1; i <= scales; i++) { 
            if (method == 1 || method == 2) {
                ...
                float U_m = calculate_U_m(coarse_data, m ,r, cols, rows, num_files/i, delta, fuzzy, distance_type, 0);
                float U_m_plus_one = calculate_U_m(coarse_data, m, r, cols, rows, num_files/i, delta, fuzzy, distance_type, 1);
                float n = negative_logarithm(U_m, U_m_plus_one);
                n_values[i-1] = n;
            }
    }
