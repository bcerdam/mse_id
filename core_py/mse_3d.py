...
def run_c_program(csv_path, scales, m, r, fuzzy, method, delta=0.7, distance_type=0, m_distance=2, sampleo=1):
    ...
    command = [os.path.join(os.path.dirname(os.getcwd()), 'core_c', 'executables', 'mse_3d'), csv_path, str(scales), str(m), str(r), str(fuzzy), str(method),
               str(delta), str(distance_type), str(m_distance), str(sampleo), str(info[0]), str(info[1]), str(info[2])]
...
def mse_3d(folder_path, scales, m, r, fuzzy, method, delta=0.7, distance_type=0, m_distance=2, sampleo=1):
    # Fuzzy params
    if fuzzy == True:
        fuzzy = 1
    else:
    ...
    for filename in os.listdir(folder_path):
        ...
        mse_values.append([filename, run_c_program(file_path, scales, m, r, fuzzy, method, delta, distance_type, m_distance, sampleo)])
...
# White Noise
# v = mse_3d('/home/bcm/Desktop/Repo/c_mse_3D/Datos/10x10x100_2/white_noise', 20, 2, 0.25, False, 'MSE')
# utils.plot_arrays(v)

# Todos
# v = mse_3d('/home/bcm/Desktop/Repo/c_mse_3D/Datos/10x10x100_2/test', 20, 2, 0.25, False, 'MSE')
# utils.plot_arrays(v)
